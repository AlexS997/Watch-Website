...
import { AlertCircle, Check, CircleDollarSign, Clock, Mail, MapPin, Phone, Send, ShoppingCart, User } from 'lucide-react';
  ...
  return (
    <div className={`${contactPageStyles.creativeCardBase} ${accentBg}`}>
      ...
      <div className="mt-4">
        <button
          onClick={ctaOnClick}
          className={`${contactPageStyles.creativeCardButtonBase} ${buttonClass} cursor-pointer`}
        >
          {ctaText}
        </button>
      </div>
...
const ContactPage = () => {
  ...
  const WHATSAPP_NUMBER = "0011225555";
          ...
          <div className={contactPageStyles.mainGrid}>
              ...
              <div className={contactPageStyles.formCard}>
                <form onSubmit={handleSubmit} className={contactPageStyles.form}>
                  <div className={contactPageStyles.inputGrid}>
                    <InputWithIcon icon={<User className='w-5 h-5 text-gray-600' />} label="Your Name" name='name' value={form.name} onChange={handleChange} placeholder="Full Name" error={errors.name} required />

                    <InputWithIcon icon={<Mail className='w-5 h-5 text-gray-600' />} label="Email" name='email' value={form.email} onChange={handleChange} placeholder="name@example.com" error={errors.email} required />
                  </div>

                  <div className={contactPageStyles.inputGrid}>
                    <InputWithIcon icon={<Phone className='w-5 h-5 text-gray-600' />} label="Phone" name='phone' value={form.phone} onChange={handleChange} placeholder="0001112222" error={errors.phone} required />

                    <SelectWithIcon icon={<Clock className='w-5 h-5 text-gray-600' />} label="Preferred Contact" name='contactMethod' value={form.contactMethod} onChange={handleChange} options={["WhatsApp", "Phone Call", "Email"]} error={errors.contactMethod} required />
                  </div>
                  
                  <div>
                    <SelectWithIcon icon={<ShoppingCart className='w-5 h-5 text-gray-600' />} label="Product of interest" name='product' value={form.product} onChange={handleChange} options={products} error={errors.product} required />
                  </div>

                  <div className={contactPageStyles.inputGrid}>
                    <InputWithIcon icon={<CircleDollarSign className='w-5 h-5 text-green-600' />} label="Estimated Budget" name='budget' value={form.budget} onChange={handleChange} placeholder="e.g. €50.000 - €150.000" error={errors.budget} required />
                    
                    <div>
                      <label className={contactPageStyles.inputLabel}>
                        Short Message{" "}
                        <span className={contactPageStyles.requiredStar}>
                          *
                        </span>
                      </label>

                      <textarea name="message" value={form.message} onChange={handleChange} rows={4} className={`${contactPageStyles.textareaContainer} ${errors.message ? contactPageStyles.inputError : contactPageStyles.inputNormal}`} placeholder='Tell us what you are looking for...'></textarea>
                    </div>
                  </div>
            ...
            <div className={contactPageStyles.rightColumn}>
              <div className={contactPageStyles.rightColumnGrid}>
                <CreativeCard title="Showroom Visits" subtitle="Private viewings by appointment" icon={<MapPin className='w-6 h-6 text-gray-600' />} ctaText="Book Visit"
                ctaOnClick={() => {
