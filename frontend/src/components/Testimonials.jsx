...
const Testimonials = () => {
  ...
  const ensureRafRunning = () => {
    ...
    const loop = (now) => {
      ...
      const next = current + (targetScroll.current - current) * 0.05;
      el.scrollLeft = next;
  ...
  return (
    <section className={testimonialPageStyles.pageSection} >
        <div className={testimonialPageStyles.container}>
            ...
            <div ref={scroller} className={testimonialPageStyles.scroller} onMouseDown={onMouseDown} onMouseLeave={onMouseLeave} onMouseUp={onMouseUp} onMouseMove={onMouseMove} onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={onTouchEnd} style={{WebkitOverflowScrolling: 'touch', touchAction: "pan-y"}} >
                {cards.map(c => (
